...
# ----------------------------------------------------------------------------
# Verbosity / debug controls
# ----------------------------------------------------------------------------
# VERBOSE_STEP: when True, write the per-step timing lines ('Time: ...',
# 'time_extend: ...', etc.) to the log on every internal step. These bloat the
# log on long runs, so keep False for production and set True to debug/profile.
# The periodic segment/branch-count progress line is printed regardless.
VERBOSE_STEP = False

# DIFFUSION_CONSERVATION_CHECK_INTERVAL: validation safeguard for the external
# diffusion solver. It copies the concentration grid and runs a reduction on
# each checked sub-step, so it is off by default.
#   0  -> disabled
#   N>0-> check every Nth diffusion sub-step (records fail count / max error)
DIFFUSION_CONSERVATION_CHECK_INTERVAL = 0

#manual input of the path:
#cwd_path ='D:/Fungi_HPCC/PLAN FOR MANY SUBMISSIONS/ENVIRONMENT 0/Hyphae_NoFusion_01'

# Resolve to the current working directory at runtime
cwd_path = os.getcwd()
...
def driver_singleNutrient(run):
    ...
    try:
        ...
        if (isPatchyExtEnvironment == 0):
            ...
            glucose_mask = np.ones_like(sub_e_gluc, dtype=bool)
        else:
        ...
        try:
            ...
            fallback_grid_file = "Results/{}/Run{}/grid_coordinates_temp_run{}_pid{}.txt".format(param_string, run, run, os.getpid())
            print("Permission denied writing grid coordinates to {}; using {}".format(grid_file, fallback_grid_file))
            thisfile = open(fallback_grid_file,'w')
        with thisfile:
        ...
        count_Lnet = []  # L_net: shortest path distance to farthest tip
        count_Leuc = []  # L_euc: Euclidean distance to farthest tip
        count_route_factor = []  # Route factor q = L_net / L_euc
        ...
        diffusion_step_count = 0   # total diffusion sub-steps (drives sampling)
        diffusion_check_count = 0  # sub-steps on which the check actually ran
        ...
        # Define N for annulus calculations (used later for trehalose tracking)
        N = round(len(x_vals)/2)-2
        ...
        while current_time < params['final_time']: 
            ...
            while(this_time < ntimes):
                ...
                if current_step % step_size_extern == 0:  
                    ...
                    if (glucDiff != 0):
                        ...
                        if (isPatchyExtEnvironment == 1):
                            # Patchy environment: mass-conserving finite volume
                            # solver with no-flux boundaries at the glucose mask.
                            sub_e_gluc_new = nf.finite_volume_diffusion_2d(
                                sub_e_gluc, params['dt_e'], params, glucose_mask)
                            sub_e_gluc[glucose_mask] = sub_e_gluc_new[glucose_mask]
                        else:
                            # Homogeneous environment: alternating-direction
                            # implicit (ADI) solver.
            ...
            # if current_step % (1*160) == 0:
                ...
                # Calculate L_net, L_euc, and route factor q
                Lnet_result = hf.calculate_Lnet_to_farthest_tip(mycelia, num_total_segs)
                Leuc_result = hf.calculate_Leuc_to_farthest_tip(mycelia, num_total_segs)
                Lnet = Lnet_result['Lnet_max']
                Leuc = Leuc_result['Leuc_max']
                route_factor = Lnet / Leuc if Leuc > 0 else 1.0
                count_Lnet.append(Lnet)
                count_Leuc.append(Leuc)
                count_route_factor.append(route_factor)

                # Save L_net metrics to CSV at each output timestep
                hf.save_Lnet_metrics_csv(Lnet_result, Leuc_result, folder_string, param_string,
                ...
                if diffusion_check_count > 0:
                    ...
                    print('Diffusion conservation: '
                          f'{diffusion_check_count} checked, '
                          f'{diffusion_fail_count} failed, '
                          f'{success_rate:.2f}% ok, '
                          f'max rel error {max_diffusion_error:.2e}')
                ...
                # Calculate and save branch spatial ratios
                spatial_ratio_result = hf.calculate_branch_spatial_ratios(mycelia, num_total_segs, origin=None)
                hf.save_branch_spatial_ratios_csv(spatial_ratio_result, folder_string, param_string, 
                                                  current_time, run, params)
        ...
        if len(min_seg_length_nonTipIdx2) > 0:
        ...
        # Calculate final L_net, L_euc, and route factor q
        Lnet_result = hf.calculate_Lnet_to_farthest_tip(mycelia, num_total_segs)
        Leuc_result = hf.calculate_Leuc_to_farthest_tip(mycelia, num_total_segs)
        Lnet = Lnet_result['Lnet_max']
        Leuc = Leuc_result['Leuc_max']
        ...
        # Save final L_net metrics to CSV
        hf.save_Lnet_metrics_csv(Lnet_result, Leuc_result, folder_string, param_string,
                                 current_time, run, params)
        ...
        count_Lnet = np.array(count_Lnet)
        count_Leuc = np.array(count_Leuc)
        count_route_factor = np.array(count_route_factor)
        avg_treha_annulus = np.array(avg_treha_annulus)
        max_treha_annulus = np.array(max_treha_annulus)
        min_treha_annulus = np.array(min_treha_annulus)
       ...
       # Print out results:
        ...
        if diffusion_check_count > 0:
            success_rate = ((diffusion_check_count - diffusion_fail_count) / 
                           diffusion_check_count * 100)
            print('Final diffusion conservation summary:')
            print(f'  steps checked: {diffusion_check_count}')
            print(f'  failed checks: {diffusion_fail_count}')
            print(f'  success rate: {success_rate:.2f}%')
            print(f'  max relative error: {max_diffusion_error:.2e}')

            if diffusion_fail_count == 0:
                status = "no violations"
            elif success_rate >= 99.9:
                status = "minor numerical errors"
            elif success_rate >= 95.0:
                status = "frequent numerical errors"
            else:
        ...
        output_dict = {
            ...
            'Lnet' : count_Lnet,
            'Leuc' : count_Leuc,
            'route_factor' : count_route_factor,
            'max_radii' : count_max_radii,
            'avg_radii' : count_avg_radii,
            'rms_radii' : count_rms_radii,
            'rms_tip_radii' : count_rms_tip_radii,
        ...
        # Calculate and save branch spatial ratios at final time
        spatial_ratio_result = hf.calculate_branch_spatial_ratios(mycelia, num_total_segs, origin=None)
        hf.save_branch_spatial_ratios_csv(spatial_ratio_result, folder_string, param_string, 
                                          current_time, run, params)
        ...
        # Plot L_net vs time
        hf.plot_Lnet_vs_time(np.array(count_times), np.array(count_Lnet),
                            folder_string, param_string, params, run)
        
        # Plot L_net vs Euclidean distance comparison
        hf.plot_Lnet_vs_euclidean(np.array(count_times), np.array(count_Lnet), np.array(count_Leuc),
                                  folder_string, param_string, params, run)
        
        # Plot Route Factor (q = L_net / L_euc) vs time
        hf.plot_route_factor_vs_time(np.array(count_times), np.array(count_route_factor),
                                     folder_string, param_string, params, run)
        ...
        print(f"Simulation run {run} completed")
