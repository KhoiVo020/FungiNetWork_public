...
# ----------------------------------------------------------------------------
# The translocation and uptake routines print several diagnostic lines on every
# internal time step. These carry no scientific value and bloat the log on long
# runs, so they are suppressed by default. Set True for verbose output.
VERBOSE = False
...
def finite_volume_diffusion_2d(sub_e, time_step, params, glucose_mask):
    ...
    Only updates where glucose_mask is True. No-flux at mask boundaries.
    ...
    delta = np.zeros_like(sub_e)

    # Horizontal edges: flux from [i,j] to [i,j+1]
    flux = r * (sub_e[:, :-1] - sub_e[:, 1:])
    flux *= glucose_mask[:, :-1] & glucose_mask[:, 1:]
    delta[:, :-1] -= flux
    delta[:, 1:]  += flux

    # Vertical edges: flux from [i,j] to [i+1,j]
    flux = r * (sub_e[:-1, :] - sub_e[1:, :])
    flux *= glucose_mask[:-1, :] & glucose_mask[1:, :]
    delta[:-1, :] -= flux
    delta[1:, :]  += flux
...
def distance_to_tip(mycelia, num_total_segs):
    ...
    # Distance to tip via multi-source BFS outward from every tip over the
    # nbr_idxs graph, computed with a queue in O(N + edges). dtt[i] is the
    # number of hops along nbr_idxs from segment i to the nearest tip; tips are
    # 0 and null (branch_id == -1) segments are 1e12.

    # Initialize dist to tip as all -1 (== "not yet assigned")
    dtt = -1*np.ones((num_total_segs,1))
    branch_id = mycelia['branch_id'][:num_total_segs].reshape(-1)
    null_segs = np.where(branch_id == -1)[0]
    # If a segment is a tip, it has a distance to tip of 0
    tip_segs = np.where(mycelia['is_tip'][:num_total_segs].reshape(-1))[0]
    dtt[tip_segs] = 0
    # Null segments act as "unreachable" sentinels. They are never enqueued
    # below, so (exactly as in the original) distance never propagates through
    # them.
    dtt[null_segs] = 1e12

    nbr_idxs = mycelia['nbr_idxs']
    # Seed the BFS with every tip (all at distance 0).
    queue = deque(int(t) for t in tip_segs)
    while queue:
        cur = queue.popleft()
        next_dist = dtt[cur, 0] + 1
        for nb in (nbr_idxs[cur] or []):
            # Only segments still at -1 are unvisited & reachable; tips (0) and
            # null segments (1e12) are already set and are skipped.
            if dtt[nb, 0] == -1:
                dtt[nb, 0] = next_dist
                queue.append(int(nb))
...
def distance_to_tip_new(mycelia, num_total_segs):
    ...
    if allow_reverse_transport == 1:
        ...
        # Precompute, once, whether each branch still contains a tip. is_tip
        # and branch_id do not change while this function runs, so a single pass
        # to build a {branch_id: has_tip} lookup makes the per-segment test O(1).
        branch_ids_flat = mycelia['branch_id'][:num_total_segs].reshape(-1)
        is_tip_flat = mycelia['is_tip'][:num_total_segs].reshape(-1)
        branch_has_tip = {}
        for j in range(num_total_segs):
            b = int(branch_ids_flat[j])
            if is_tip_flat[j]:
                branch_has_tip[b] = True
            elif b not in branch_has_tip:
                branch_has_tip[b] = False
            ...
            for idx in range(num_total_segs):
                ...
                if dtt[idx] == -1 and (current_dist-1) in dtt[mycelia['nbr_idxs'][idx]]:
                    ...
                    if len(lead_dist) == 1 and (mycelia['branch_id'][idx] not in mycelia['branch_id'][mycelia['nbr_idxs'][idx][lead_dist[0]]]):
                        # Assign only if idx's branch contains no tip (was the
                        # all(is_tip==False) test); otherwise defer to a later
                        # layer, exactly as before.
                        if not branch_has_tip[int(mycelia['branch_id'][idx])]:
                            dtt[idx] = current_dist
                        else:
                            continue
                    else:
...
def transloc(mycelia, params, num_total_segs, dtt, isActiveTrans, whichInitialCondition,
    ...
    # Per-step diagnostic block: pure logging, no effect on state. Gated so it
    # does not flood the log file (and skip the ndensity list build) in long runs.
    if VERBOSE:
...
def uptake(sub_e_gluc, mycelia, num_total_segs, var_nutrient_backgrnd, time_step, params):
    ...
    # Per-uptake-step mass-conservation diagnostics, gated to keep them out of
    # the log on long runs.
    if VERBOSE:
