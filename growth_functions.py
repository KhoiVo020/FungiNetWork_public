...
# ----------------------------------------------------------------------------
# Verbosity / debug logging control
# ----------------------------------------------------------------------------
# Every print() in this module is mirrored to the simulation log file by the
# driver's logger. The per-step timing print()s below fire on every internal
# step and carry no scientific information, so they are suppressed by default.
# Set this to True to restore the verbose per-step timing output.
...
def count_hyphae(mycelia, size, num_total_segments):
    # Histogram of how many segments map to each external grid cell.
    count = np.zeros((size, size), dtype=int)
    cells = mycelia['xy_e_idx'][:num_total_segments].astype(int)
    if cells.shape[0] > 0:
        # np.add.at handles repeated indices correctly (unlike count[idx]+=1)
...
def _grid_cell_of(mycelia, idx, x_vals, y_vals):
    """Return the (ix, iy) external-grid cell that segment ``idx`` maps to.

    Uses the *start* endpoint (xy1) of the segment, exactly as the original
    map_to_grid did. A segment's start endpoint is fixed once the segment is
    created (extension only moves the far endpoint xy2; splitting/branching
    create brand-new segments), so each segment belongs to a single grid cell
    for its whole lifetime. That immutability is what makes the append-only
    hash-index bookkeeping below correct.
    """
    ix = int(np.argmin(np.abs(mycelia['xy1'][idx, 0] - x_vals)))
    iy = int(np.argmin(np.abs(mycelia['xy1'][idx, 1] - y_vals)))
    return ix, iy


def _ensure_grid_cell_map(mycelia, num_total_segs):
    """Lazily (re)build the cell -> [segment indices] index if it is missing.

    Normally 'grid_cell_map' is created in setup_functions and kept current
    incrementally. This fallback only fires for simulation states that were
    built/loaded without it (e.g. an old pickle/restart); it reconstructs the
    index from the already-computed xy_e_idx table so the rest of the code can
    rely on it being present.
    """
    if mycelia.get('grid_cell_map') is None:
        cell_map = {}
        for j in range(num_total_segs):
            cell = (int(mycelia['xy_e_idx'][j, 0]), int(mycelia['xy_e_idx'][j, 1]))
            cell_map.setdefault(cell, []).append(j)
        mycelia['grid_cell_map'] = cell_map
    return mycelia


def _register_on_grid(mycelia, idx, num_total_segs, x_vals, y_vals):
    """Register a single segment on the external grid in O(1) (amortized).

    'grid_cell_map' maps each occupied cell -> list of segment indices in it,
    so registering a segment is a dict lookup plus a few appends, independent
    of the total segment count.

    'share_e[idx]' is the list of all segments sharing idx's cell (idx itself
    included), and the relation is symmetric (every co-located segment lists
    every other). share_e is only consumed for diagnostics (a max/mean segment
    density print) and for density filters that are currently commented out,
    so building it this way does not change the simulation dynamics. Counts
    here are exact (no segment is listed more than once).
    """
    _ensure_grid_cell_map(mycelia, num_total_segs)
    cell = _grid_cell_of(mycelia, idx, x_vals, y_vals)
    mycelia['xy_e_idx'][idx, :] = cell

    members = mycelia['grid_cell_map'].setdefault(cell, [])
    # idx joins the cell's clique: every existing member gains idx ...
    for m in members:
        mycelia['share_e'][m].append(idx)
    # ... and idx's own list is the existing members plus itself.
    mycelia['share_e'][idx] = members + [idx]
    members.append(idx)
    return mycelia
...
def map_to_grid(mycelia, idx, num_total_segs, x_vals, y_vals):
    ...
    coordinates and record which segments share that grid point. Now backed by
    the O(1) grid_cell_map index (see _register_on_grid) instead of an O(N) scan.
    """
    return _register_on_grid(mycelia, idx, num_total_segs, x_vals, y_vals)
...
def map_to_grid2(mycelia, new_tips, num_total_segs, x_vals, y_vals):
    ...
    mycelia : dictionary
        Stores structural information of mycelia colony for all hyphal segments.
    new_tips : iterable of int
        Indices of the newly created segments to register on the grid.
    num_total_segs : int
        Current total number of segments in the mycelium.
    x_vals : list/array
        The x-values of the external grid.
    ...
    Purpose
    -------
    Batch version of map_to_grid for a set of new tips. O(k) for k new tips via
    the grid_cell_map index, versus the old O(k*N) double-scan.
    """
    t0 = time.time()
    for idx in new_tips:
        _register_on_grid(mycelia, int(idx), num_total_segs, x_vals, y_vals)
    if VERBOSE_TIMING:
        print('\tTime to map to grid2 : ', time.time() - t0)
    return mycelia
...
def split_segment(mycelia, num_total_segs, x_vals, y_vals, isCalibration, dist2Tip_new, params):
    ...
    if VERBOSE_TIMING:
        print('\tTime to update neighbor list : ', time.time() - t0)
    ...
    # Map to external grid. Register each new tip exactly once.
